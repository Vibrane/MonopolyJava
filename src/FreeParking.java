...
public class FreeParking extends PropertyCard {

    private DrawingCanvas canvas;
    private FramedRect propertyRect;
    private String propertyName = "Free Parking";
    private FilledRect cardColor, p1, p2, p3, p4, ownedBy;
    static int value = 0;

    public FreeParking(DrawingCanvas boardCanvas) {
        this.canvas = boardCanvas;
        this.propertyRect = new FramedRect(0 + 5, 0 + 5, this.canvas.getWidth() / 11 - 5, 
        	this.canvas.getHeight() / 11 - 5, canvas);
        this.cardColor = new FilledRect(this.propertyRect.getLocation(), 
        	this.propertyRect.getWidth(), this.propertyRect.getHeight() / 3, canvas);
        this.cardColor.setColor(Color.darkGray);
