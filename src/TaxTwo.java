...
public class TaxTwo extends PropertyCard {

	private DrawingCanvas canvas;
	private FramedRect propertyRect;
	private String propertyName = "Tax Two";
	private FilledRect cardColor, p1, p2, p3, p4, ownedBy;
	private int value = 280;

	public TaxTwo(DrawingCanvas boardCanvas) {
		this.canvas = boardCanvas;
		this.propertyRect = new FramedRect((canvas.getWidth() * 10) / 11 + 5, canvas.getHeight() * 8 / 11 + 5,
	...
	public void run() {
		...
		while (true) {
			...
			this.propertyRect.moveTo((canvas.getWidth() * 10) / 11 + 5, canvas.getHeight() * 8 / 11  + 5);
