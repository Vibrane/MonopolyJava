...
public class RandomCardOne extends PropertyCard {

    private DrawingCanvas canvas;
    private FramedRect propertyRect;
    private String propertyName = "Random Card";
    private FilledRect cardColor, p1, p2, p3, p4, ownedBy;
    

    public RandomCardOne(DrawingCanvas boardCanvas) {
        this.canvas = boardCanvas;
        this.propertyRect = new FramedRect((canvas.getWidth() * 2) / 11 + 5, 0 + 5, this.canvas.getWidth() / 11 - 5, this.canvas.getHeight() / 11 - 5, canvas);
        this.cardColor = new FilledRect(this.propertyRect.getLocation(), this.propertyRect.getWidth(), this.propertyRect.getHeight() / 3, canvas);
        this.cardColor.setColor(Color.WHITE);
        this.p1 = new FilledRect(this.cardColor.getX(), this.cardColor.getY() + this.cardColor.getHeight(), this.propertyRect.getWidth() / 4, this.propertyRect.getHeight() / 3, canvas);
        this.p1.setColor(Color.BLUE);
        this.p2 = new FilledRect(this.p1.getX() + this.p1.getWidth(), this.cardColor.getY() + this.cardColor.getHeight(), this.propertyRect.getWidth() / 4, this.propertyRect.getHeight() / 3, canvas);
        this.p2.setColor(Color.RED);
        this.p3 = new FilledRect(this.p2.getX() + this.p2.getWidth(), this.cardColor.getY() + this.cardColor.getHeight(), this.propertyRect.getWidth() / 4, this.propertyRect.getHeight() / 3, canvas);
        this.p3.setColor(Color.YELLOW);
        this.p4 = new FilledRect(this.p3.getX() + this.p3.getWidth(), this.cardColor.getY() + this.cardColor.getHeight(), this.propertyRect.getWidth() / 4, this.propertyRect.getHeight() / 3, canvas);
        this.p4.setColor(Color.GREEN);
        this.ownedBy = new FilledRect(this.propertyRect.getX(), this.p4.getY() + this.p4.getHeight(), this.propertyRect.getWidth(), this.propertyRect.getHeight() / 3, canvas);
        this.ownedBy.setColor(Color.BLACK);
    ...
    public void run() {
        ...
        while (true) {            
                ...
                this.propertyRect.moveTo((canvas.getWidth() * 2) / 11 + 5, 0 + 5);
	...
	public int getValue() {
		return (int) (Math.random() * (200 - (-200))) + (-200);
	}
