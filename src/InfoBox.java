...
public class InfoBox {
	...
	private DrawingCanvas canvas;
	private FilledRect propertyRect;
	

	public InfoBox(DrawingCanvas boardCanvas) {
		this.canvas = boardCanvas;
		this.propertyRect = new FilledRect(canvas.getWidth() / 11 + 5, canvas.getHeight() * 5 / 11 + 5,
	...
	{
		if (MonopolyBoard.pCounter == 0)
		{
			this.propertyRect.setColor(Color.BLUE);
		}
		
		else if (MonopolyBoard.pCounter == 1)
			this.propertyRect.setColor(Color.RED);
		
		else if (MonopolyBoard.pCounter == 2)
			this.propertyRect.setColor(Color.YELLOW);
		else
			this.propertyRect.setColor(Color.GREEN);			
	}
